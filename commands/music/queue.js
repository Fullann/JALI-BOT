...
module.exports.run = async (bot,message,args) =>{
        ...
        if (!bot.player.getQueue(message)) return message.channel.send(`${bot.emotes.error} - Aucun son n'est lancée acctuellement !`);

        let totalTime = 0;
        message.channel.send(`**Liste du serveur - ${message.guild.name} ${bot.emotes.queue} ${bot.player.getQueue(message).loopMode ? '(en boucle)' : ''}**\nActuelle : ${queue.playing.title} | ${queue.playing.author}\n\n` + (queue.tracks.map((track, i) => {
